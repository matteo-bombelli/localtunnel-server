...
const ALL_IDS = process.env.AVAILABLE_IDS?
    `${process.env.AVAILABLE_IDS}`
        .split(",")
        .filter((el)=>!!el):
    [];

const usableId = (clients)=>{
...
class ClientManager {
    ...
    async newClient(id) {
        ...
        if (clients[id] || !(ALL_IDS.includes(id))) {
            id = (ALL_IDS.length>0)?
                usableId(this.clients):
                hri.random();
        }

        if(!id){
            throw new Error("all id already used");
        }
